...
async function deployProxyContract(name, args, label, options) {
  if (!options && typeof label === "object") {
    label = null;
    options = label;
  }
  ...
  let contract;
  if (options) {
    contract = await hre.upgrades.deployProxy(contractFactory, args, options);
  } else {
    contract = await hre.upgrades.deployProxy(contractFactory, args);
  }
  // const argStr = args.map((i) => `"${i}"`).join(" ")
  console.info(`Deploying ${info} Proxy = ${contract.address}`);
  let tx = await contract.deployTransaction.wait();
  if (label === undefined) console.info(`Completed ${info} at txHash: ${tx.transactionHash}`);
  else console.info(`Completed ${label} at txHash: ${tx.transactionHash}`);
  console.info(`========================`);
